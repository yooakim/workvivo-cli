...
dotnet publish src\WorkvivoCli.csproj -c Release -r win-x64 --self-contained -o dist\windows-x64
if %errorlevel% neq 0 exit /b %errorlevel%
echo Build complete: dist\windows-x64

echo.
echo Building for macOS Intel (osx-x64)...
dotnet publish src\WorkvivoCli.csproj -c Release -r osx-x64 --self-contained -o dist\macos-x64
if %errorlevel% neq 0 exit /b %errorlevel%
echo Build complete: dist\macos-x64

echo.
echo Building for macOS ARM (osx-arm64)...
dotnet publish src\WorkvivoCli.csproj -c Release -r osx-arm64 --self-contained -o dist\macos-arm64
if %errorlevel% neq 0 exit /b %errorlevel%
echo Build complete: dist\macos-arm64

echo.
echo Building for Linux x64 (linux-x64)...
dotnet publish src\WorkvivoCli.csproj -c Release -r linux-x64 --self-contained -o dist\linux-x64
if %errorlevel% neq 0 exit /b %errorlevel%
echo Build complete: dist\linux-x64

echo.
echo Building for Linux ARM (linux-arm64)...
dotnet publish src\WorkvivoCli.csproj -c Release -r linux-arm64 --self-contained -o dist\linux-arm64
if %errorlevel% neq 0 exit /b %errorlevel%
